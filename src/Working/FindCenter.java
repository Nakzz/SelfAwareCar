		...
		if (capture.isOpened()) {
			while (true) {
				...
				if (!webcam_image.empty()) {
					...
					Imgproc.medianBlur(webcam_image, webcam_image, 3); // cancel
																		// noise
																		// right
																		// from
																		// camera
					...
					Imgproc.findContours(thresholded2, contours, new Mat(), Imgproc.RETR_LIST,
					...
					for (int i = 0; i < contours.size(); i++) {
						...
						Core.rectangle(thresholded2, new Point(rect.x, rect.y),
								new Point(rect.x + rect.width, rect.y + rect.height), new Scalar(255, 255, 0), 3);

					}

					// Imgproc.findContours(thresholded2, contours,
					// thresholded2,
					// Imgproc.RETR_LIST,Imgproc.CHAIN_APPROX_SIMPLE);
					// Imgproc.drawContours(thresholded2, contours, -1,
					// colorCont);
					...
					//Highgui.imwrite("FInal.JPG", thresholded2); // wrote this to
																// see what the
																// updated
																// images
																// becomes
